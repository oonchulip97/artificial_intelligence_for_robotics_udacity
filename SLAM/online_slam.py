...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for k in range(len(data)):
        ...
        # expand Omega and Xi
        dim_expand = dim + 2
        Omega = Omega.expand(dim_expand,dim_expand, \
                             range(2)+range(4,dim_expand), \
                             range(2)+range(4,dim_expand))
        Xi = Xi.expand(dim_expand,1,range(2)+range(4,dim_expand),[0])
        ...
        # retrieve truncated Omega and Xi
        A = Omega.take(range(2),range(2,dim_expand))
        B = Omega.take(range(2),range(2))
        C = Xi.take(range(2),[0])
        Omega2 = Omega.take(range(2,dim_expand),range(2,dim_expand))
        Xi2 = Xi.take(range(2,dim_expand),[0])
