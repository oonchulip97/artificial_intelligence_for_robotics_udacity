...
def run(grid, goal, spath, params, printflag = False, speed = 0.1, timeout = 1000):
    ...
    while not myrobot.check_goal(goal) and N < timeout:
        ...
        ### ENTER CODE HERE
        robot_x = estimate[0] - spath[index][0]
        robot_y = estimate[1] - spath[index][1]
        delta_x = spath[index+1][0] - spath[index][0]
        delta_y = spath[index+1][1] - spath[index][1]
        u = (robot_x * delta_x + robot_y * delta_y)/(delta_x ** 2 + delta_y ** 2)
        if u > 1:
            index += 1
            index = index % len(spath)
            delta_x = spath[(index+1)%len(spath)][0] - spath[index][0]
            delta_y = spath[(index+1)%len(spath)][1] - spath[index][1]
        cte = (robot_y * delta_x - robot_x * delta_y)/sqrt(delta_x ** 2 + delta_y ** 2)
        # ----------------------------------------
