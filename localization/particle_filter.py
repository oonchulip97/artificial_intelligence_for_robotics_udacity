...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        # ADD CODE HERE
        moved_robot = robot(self.length)
        moved_robot.set_noise(self.bearing_noise, self.steering_noise, self.distance_noise)

        steering_angle = motion[0] + random.gauss(0,self.steering_noise)
        moved_dist = motion[1] + random.gauss(0,self.distance_noise)

        turned_angle = moved_dist / self.length * tan(steering_angle)
        if abs(turned_angle) < 0.001:
            moved_robot.x = self.x + moved_dist * cos(self.orientation)
            moved_robot.y = self.y + moved_dist * sin(self.orientation)
            moved_robot.orientation = (self.orientation + turned_angle) % (2*pi)

        else:
            radius = moved_dist / turned_angle
            center_x = self.x - sin(self.orientation) * radius
            center_y = self.y + cos(self.orientation) * radius

            moved_robot.x = center_x + sin(self.orientation + turned_angle) * radius
            moved_robot.y = center_y - cos(self.orientation + turned_angle) * radius
            moved_robot.orientation = (self.orientation + turned_angle) % (2*pi)

        return moved_robot # make sure your move function returns an instance
    ...
    def sense(self, have_bearing_noise = 1): #do not change the name of this function
        Z = []

        # ENTER CODE HERE
        # HINT: You will probably need to use the function atan2()
        for i in range(len(landmarks)):
            dist_x = landmarks[i][1] - self.x
            dist_y = landmarks[i][0] - self.y

            Z.append(((atan2(dist_y,dist_x) - self.orientation) + \
                    + have_bearing_noise * random.gauss(0,self.bearing_noise)) % (2*pi))
